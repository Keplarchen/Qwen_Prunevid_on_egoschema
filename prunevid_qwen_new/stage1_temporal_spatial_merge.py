...
class SpatialTemporalTokenMerger(nn.Module):
    ...
    def forward(
        ...
        # 处理batch（当前假设batch_size=1）
        # 多batch支持需要循环处理每个batch
        if batch_size != 1:
            raise NotImplementedError("当前实现只支持batch_size=1，多batch支持将在未来版本添加")

        # [batch=1, num_visual, hidden_dim] -> [num_visual, hidden_dim]
        visual_tokens_single = visual_tokens[0]
        visual_position_ids_single = visual_position_ids[:, 0, :]  # [3, num_visual]
        ...
        for seg_idx, seg_frames in enumerate(segments):
            ...
            if num_static > 0:
                ...
                static_pos_cos = visual_cos[:, 0, static_indices, :]  # [3, num_static, head_dim]
                static_pos_sin = visual_sin[:, 0, static_indices, :]
            ...
            if num_dynamic > 0:
                ...
                for frame_idx_in_seg in range(seg_T):
                    ...
                    dynamic_pos_cos = visual_cos[:, 0, dynamic_indices, :]
                    dynamic_pos_sin = visual_sin[:, 0, dynamic_indices, :]
        ...
        if self.verbose:
            print(f"\n[Stage 1] 最终合并结果: {num_visual_tokens} -> {num_merged} tokens")
            print(f"  压缩率: {num_merged/num_visual_tokens*100:.1f}%")
            print(f"  减少率: {(1-num_merged/num_visual_tokens)*100:.1f}%")

        # 步骤5：重建完整序列（文本token + 合并后的视觉token）
        # [batch, seq_len, hidden_dim] -> [文本前] + [视觉] + [文本后]
        text_before = hidden_states[:, :visual_token_start, :]
        text_after = hidden_states[:, visual_token_end:, :]

        # 添加batch维度
        merged_visual = all_merged_tokens.unsqueeze(0)  # [1, num_merged, hidden_dim]

        new_hidden_states = torch.cat([text_before, merged_visual, text_after], dim=1)
        ...
        all_merged_pos_ids_expanded = all_merged_pos_ids.unsqueeze(1)  # [3, 1, num_merged]

        new_position_ids = torch.cat([pos_ids_before, all_merged_pos_ids_expanded, pos_ids_after], dim=2)
        ...
        all_merged_cos_expanded = all_merged_cos.unsqueeze(1)  # [3, 1, num_merged, head_dim]
        all_merged_sin_expanded = all_merged_sin.unsqueeze(1)

        new_cos = torch.cat([cos_before, all_merged_cos_expanded, cos_after], dim=2)
        new_sin = torch.cat([sin_before, all_merged_sin_expanded, sin_after], dim=2)

        new_position_embeddings = (new_cos, new_sin)

        # 统计信息
        stats = compute_compression_stats(num_visual_tokens, num_merged)
        stats.update({
